...
export async function status(): Promise<void> {
  ...
  const regularStatus: MigrationStatusItem[] = regularFiles.map(file => ({
    name: file.name,
  ...
  const oneOffStatus: MigrationStatusItem[] = oneOffFiles.map(file => ({
    name: file.name,
    applied: appliedSet.has(file.name),
  }));
 ...
 */
function displayMigrationTable(migrations: MigrationStatusItem[]): void {
